...
const createGame = (player1Socket, player2Socket) => {
  ...
  player1Socket.on('disconnect', () => {
    games[gameIndex].gameState.winner = GameService.game.setWinner(games[gameIndex], 'player:2', "Your opponent has left");
  ...
  player2Socket.on('disconnect', () => {
    games[gameIndex].gameState.winner = GameService.game.setWinner(games[gameIndex], 'player:1', "Your opponent has left");
    updateClientsViewEndGame(games[gameIndex], false, false);
...
io.on('connection', socket => {
  ...
  socket.on('game.grid.selected', (data) => {
    ...
    if (games[gameIndex].gameState.currentTurn === 'player:1') {
      games[gameIndex].gameState.player1Info.tokens--;
      games[gameIndex].gameState.player1Info.score = score.score;
    } else {
      games[gameIndex].gameState.player2Info.tokens--;
    ...
    } else if (games[gameIndex].gameState.player1Info.tokens == 0 || games[gameIndex].gameState.player2Info.tokens == 0) {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        if (game.gameState.choices.availableChoices.length == 0) {
          console.log("Bot had no choices on first roll");
          rollBot(game);

          setTimeout(() => {
            if (game.gameState.choices.availableChoices.length == 0) {
              console.log("Bot had no choices on second roll");
              rollBot(game);

              setTimeout(() => {
                if (game.gameState.choices.availableChoices.length == 0) {
                  console.log("Bot had no choices on third roll");
...
const selectGridBot = (game, selectedChoice) => {
  ...
  if (game.gameState.currentTurn === 'player:1') {
    game.gameState.player1Info.tokens--;
    game.gameState.player1Info.score = score.score;
  } else {
    game.gameState.player2Info.tokens--;
  ...
  } else if (game.gameState.player1Info.tokens == 0 || game.gameState.player2Info.tokens == 0) {
